...
#Y~X1+X2+X3+.....
#Y=sales quantity discrete quantities
#fit2 = lm(formula = sales ~ price + promotion, data=omni)
fit2 = lm(formula = sales ~ price + promotion, data=omni3)
#to remove outliers
#fit3 = lm(formula = sales ~ price + promotion, data=omni3[-c(11,14,15),])
# summary statistics of model IMP STEP
summary(fit2)

#A model with above 0.6 and between 0.6 to 0.9 is a good model. We use adjusted R square value. 
...
#prediction can be done by intrapolation only and not extrapolation .
...
omni3[14,]
omni3[-c(11,14,15)]
#after removing outliers, create new model and then plot again
