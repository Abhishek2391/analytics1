...
str(Groceries)
Groceries
inspect(Groceries[1:5])  #view #list of transactions
LIST(Groceries[1:6])  #another view

#Find Frequent Itemset
#eclat tells frquntly purchased items
#support is a minimum threshold level: =0.01 means the combination of items should have been bought 0.01* total transactions times. So these items become frequntly purchased items.
#maxlen should be kept low as the algo is heavy and can hang the machine. 
#minlen means minimum how many items and maxlen means maximum how many items
...
#relative plot means in terms of percentage
itemFrequencyPlot(Groceries, topN = 10, type='relative')
abline(h=0.15)

# Create rules and the relationship between items
#parameters are min filter conditions 
#apropri is used to calculate confidence values and Creating rules. Can specify minimum support. supp=0.005, means the items has been purchased 0.005 times the total transaction. All those who bought item x , 50% has bought y condifently, this is what u can decipher form conf=0.5. minimum 2 items on both left and right hand side.
#rules = apriori(Groceries, parameter = list(supp = 0.005, conf = 0.5, minlen=2))
rules = apriori(Groceries, parameter = list(supp = 0.01, conf = 0.5, minlen=2,maxlen=3))
rules

inspect (rules[1:5])
#Sort Rules by confidence, lift and see the data. Looking for high confidence
...
#Find out what events were influenced by a given event. rhs is the causing event and the lhs is the impact occured
